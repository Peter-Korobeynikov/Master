...
if($mode=='change_choosen_shipping') {
    ...
    if(!empty($_REQUEST['comment'])) {
        $cart['customer_notes']=$_REQUEST['comment'];
    }
    ...
    if (!empty($shipping_id)) {
        fn_set_notification('N', __('notice'), __('salesbeat_shipping_changed'));
    }
  ...
  return array(CONTROLLER_STATUS_OK, 'checkout.' . $_REQUEST['redirect_mode']);
    ...
    if(!empty($_REQUEST['q'])) {
        $result=fn_salesbeat_request('https://app.salesbeat.pro/api/v1/get_cities?city='.rtrim(urlencode($_REQUEST['q'])),array(),'GET');
