...
/**
 * Provides customer location hash for caching purposes.
 *
 * @return string
 *
 * @internal
 */
function fn_warehouses_blocks_get_customer_location_hash()
{
    /** @var \Tygh\Location\Manager $location */
    $location = Tygh::$app['location'];

    $customer_location = $location->getLocation()->toArray();
    $customer_location_hash = md5(json_encode($customer_location));

    return $customer_location_hash;
}
