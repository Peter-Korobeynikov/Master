...
function fn_newsletters_get_recipients(array $params)
{
    ...
    if (!empty($params['mailing_lists'])) {
        $list_recipients = db_get_array(
            'SELECT 0 as user_id, subscribers.email, subscribers.lang_code FROM ?:subscribers AS subscribers'
    ...
    if (!empty($params['users'])) {
        ...
        $user_recipients = db_get_array('SELECT users.user_id, users.email, users.lang_code FROM ?:users AS users WHERE users.user_id IN (?n)', $users);
    }
    ...
    if (!empty($params['abandoned_days'])) {
        ...
        $abandoned_recipients = db_get_array(
            'SELECT users.user_id, users.email, users.lang_code FROM ?:users AS users'
...
function fn_newsletters_get_send_list($key, $limit, $offset)
{
    ...
    return db_get_hash_multi_array(
        'SELECT newsletter_batch_recipients.*'
