 ...
 * @param null   $whitelist_of_logo_types
 *
...
function fn_create_theme_logos_by_layout_id(
    ...
    $repo_dest = fn_get_theme_path('[themes]/' . $theme_name, 'C', $company_id, false, $storefront_id);
    ...
    foreach ($logo_types as $type => $logo) {
        ...
        $logo_data = [
            'type'       => $type,
            'layout_id'  => empty($logo['for_layout'])
                ? 0
                : $layout_id,
            'image_path' => $image_path,
            'style_id'   => empty($logo['single_logo'])
                ? $style_id
                : '',
        ];
