(function(_, $) {
    $(_.doc).ready(function() {
        ...
        $(_.doc).on('click', '.cm-sdek-select-location', function () {
            ...
            if (!$('[data-ca-pickup-select-office]' +
                '[data-ca-shipping-id="' + shipping_id + '"]' +
                '[data-ca-group-key="' + group_key + '"]' +
                '[data-ca-location-id="' + location + '"]').length) {
                    
                delete_dummy_elm_after_calculate = true;
                var $shipping_item_elm = $('<input>')
                    .addClass('hidden')
                    .attr('type', 'radio')
                    .attr('name', 'select_office[' + group_key + '][' + shipping_id + ']')
                    .val(location)
                    .attr('data-ca-pickup-select-office', true)
                    .attr('data-ca-shipping-id', shipping_id)
                    .attr('data-ca-group-key', group_key)
                    .attr('data-ca-location-id', location);
