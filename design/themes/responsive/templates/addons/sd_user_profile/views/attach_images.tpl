...
(function(_, $) {
    $.ceEvent('on', 'ce.delete_image', function(r, p) {
        if (r.deleted == true) {
            $("[id^=det_img_]").replaceWith('<span class="ty-no-image" style="height: 120px;"><i class="ty-no-image__icon ty-icon-image" title="' + _.tr('no_image') + '"></i></span>');
...
<div id="box_attach_images_{$name}_{$key}" class="attach-images">
    ...
    {/if}
        ...
        <div class="pull-left image-wrap">
            ...
            <a data-ca-target-id="image_{$pair.image_id}" href="{"image.delete_image?pair_id=`$pair.pair_id`&image_id=`$pair.image_id`&object_type=`$object_type`"|fn_url}" class="image-delete cm-confirm cm-post cm-ajax delete cm-delete-image-link cm-tooltip" data-ca-event="ce.delete_image" title="{__("remove")}">{__("remove")}</a>
            {/if}
            {/if}
            {if !$hide_images}
                <div class="image">
