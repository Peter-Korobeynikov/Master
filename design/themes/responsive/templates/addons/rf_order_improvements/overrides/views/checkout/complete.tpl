...
{if $smarty.session.rf_registered_password}
    {__('rf_we_have_created_profile', ['[password]' => $smarty.session.rf_registered_password, '[email]' => $order_info.email])}
{/if}

{if $order_info && $settings.Checkout.allow_create_account_after_order == "Y" && !$auth.user_id && !$smarty.session.rf_registered_password}
    ...
    {if $smarty.session.rf_order_linked}
        {__('rf_we_have_linked_order')}
    {/if}
