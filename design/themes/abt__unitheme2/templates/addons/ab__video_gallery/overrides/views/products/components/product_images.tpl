...
{capture name="abt__ut2_vertical_gallery_width"}{if ($product.image_pairs|count + $ab__vg_videos|count) >= 6 && $settings.Appearance.thumbnails_gallery == "N"}{$th_size*2+24}{else}{$th_size+12}{/if}{/capture}
...
{if $smarty.capture.$product_labels|trim}
	<div class="ab-{if $is_vertical}vg-{/if}{if $settings.Appearance.thumbnails_gallery == "Y"}j-{/if}product_labels" {if $product.image_pairs && $is_vertical && ($product.image_pairs|count + $ab__vg_videos|count) >= 1}style="margin-left: {$smarty.capture.abt__ut2_vertical_gallery_width + 10}px"{/if}>{$smarty.capture.$product_labels nofilter}</div>
{/if}

<div id="product_images_{$preview_id}" class="ty-product-img cm-preview-wrapper {if $is_vertical}ab-{if $settings.Appearance.thumbnails_gallery == "Y"}vg-{/if}vertical{/if}" {if $is_vertical && ($product.image_pairs|count + $ab__vg_videos|count) >= 1}style="width: -webkit-calc(100% - {$smarty.capture.abt__ut2_vertical_gallery_width}px);width: calc(100% - {$smarty.capture.abt__ut2_vertical_gallery_width + 10}px);{if $image_height_block} max-height: {$image_height_block}px;{/if}"{elseif $image_height_block}style="max-height: {$image_height_block}px;"{/if}>
...
{if $product.image_pairs || $ab__vg_videos}
    ...
    {if $settings.Appearance.thumbnails_gallery == "Y"}
        ...
        {strip}
            <div class="ty-center ty-product-thumbnails_gallery{if $is_vertical} ab-vg-vertical-thumbnails{/if}" {if $is_vertical}style="width: {$smarty.capture.abt__ut2_vertical_gallery_width - 4}px;{if $image_height_block}max-height: {$image_height_block}px;{/if}"{/if}>
                            ...
                            {if $ab__vg_videos && $addons.ab__video_gallery.position == 'pre'}
                                {foreach $ab__vg_videos as $video}
                                    ...
                                    <div class="cm-item-gallery">
                                        <a data-ca-gallery-large-id="det_img_link_{$preview_id}_{$video.video_id}" class="ab__vg-image_gallery_item cm-gallery-item cm-thumbnails-mini ty-product-thumbnails__item gallery {if $ab__vg_settings.replace_image == "Y" && $video@iteration == 1} active{/if}" data-ca-image-order="{$image_counter}" data-ca-parent="#product_images_{$preview_id}">
                                            <img class="lazyOwl" src="{$smarty.const.AB__VG_LAZY_IMAGE}" data-src="{$images_dir}/addons/ab__video_gallery/youtube_logo.png" width="{$th_size}" height="{$th_size}" alt="{$video.title|strip_tags}" />
                                        </a>
                                    </div>
                                {/foreach}
                            {/if}
        ...
        {/strip}
    {else}
        <div class="ty-product-thumbnails ty-center{if $is_vertical} ab-{if $settings.Appearance.thumbnails_gallery == "Y"}vg-{/if}vertical-thumbnails{/if}" {if $is_vertical}style="width: {$smarty.capture.abt__ut2_vertical_gallery_width - 1}px;{if $settings.Appearance.thumbnails_gallery == "N" && $image_height_block} max-height: {$image_height_block}px;{/if}"{/if} id="images_preview_{$preview_id}">
            {strip}
